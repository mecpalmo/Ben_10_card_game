...
public class CreatingDeckWindow extends JFrame{
	...
	private class AllCardsPanel extends JPanel{
		...
		private final float RightPanelXRatio = 0.38f; // procentowa szeroko�� cz�ci panelu na w�asn� tali�
		private final float SpaceRatio = 0.05f; // procentowy odst�p mi�dzy kartami odnosz�cy si� do rozmiaru karty
		private final int MyDeckSpaceY = (int)(size_y*0.12);// odst�p pionowy od g�ry w�asnej talii
		...
		private void setBackground() {
			try {
				Background = loadImage("deck_background.png");
			} catch (IOException e) {
		...
		private void setButtons() {
			...
			CreateNewDeck = new JButton("Utwórz nową talię");
	...
	private void createNewDeck() {
		...
		while(filename.length() <=0 || filename.contains("Default")) {
			filename = JOptionPane.showInputDialog("Podaj nazwę nowej talii (nie może zawierać \"Default\")");
		}
